...
const Import = ({importPath, templatePath , handleToast}) => {
  ...
  const submit = (e) => {
    e.preventDefault()
    post(importPath, {forceFormData: true,  onSuccess: () => {handleShow(); reset(); handleToast();} });
  }
      ...
      <div className='h-[230px] w-[500px] fixed z-[52] top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-gray-100 rounded-md px-5 py-5 flex flex-col gap-4'>
        ...
        <form className='flex flex-col h-full gap-2'  onSubmit={submit}>
          <div className='flex-1 flex flex-col justify-center gap-1'>
            <span className="sr-only">Choose import file</span>
             <input id='file' type="file" onChange={e => setData('file', e.target.files[0])}  className="block w-full text-sm text-slate-500
                ...
                file:bg-secondary file:text-gray-200
          ...
          <div className='h-10 flex gap-4 justify-center items-center text-sm'>
            <button className='py-2 px-4 bg-secondary border-[0.1px] border-secondary text-white rounded-md' type="submit">Import</button>
            <button onClick={()=>{handleShow(); reset(); clearErrors();}} className='py-2 px-4 bg-gray-100 border-[0.1px] border-secondary text-secondary rounded-md' type="button">Cancel</button>
          </div>
        </form>
      </div>
    </div>
